...
1. **Shared-memory tiling for dot products** -- Tile the K/V dimension in shared memory so that each thread block reuses loaded K/V tiles across multiple query heads. Reduces global memory traffic proportional to tile reuse factor.
